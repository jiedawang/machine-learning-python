...
class MultilayerPerceptron:
    ...
    fit_result: 拟合任务结果，str类型，标识结束状态
    ...
    def bind_func_(self,activation,cost,optimizer):
        ...
        #参数值校验
        activation_list=['sigm','tanh','relu','none']
        layers=self.layers
        activation_config=[]
        ...
        if (activation_config[-1] in ('relu','none'))&(self.mode=='c'):
            print('\nwarning: you set relu/none activation of output for classification')
        if (activation_config[-1] in ('sigm','tanh'))&(self.mode=='r'):
            print('\nwarning: you set sigmoid/tanh activation of output for regression')
        cost_list=['mse','ce']
        check_limit('cost',cost in cost_list,str(cost_list))
        if (cost=='ce')&(self.mode=='r'):
            print('\nwarning: you set cross entropy cost for regression')
    ...
    def optimize_(self,X,Y,test_X=None,test_Y=None,
             monitor_cost=False,monitor_score=False):
    ...
    def load(self,file_path):
        ...
        self.bind_func_(data['activation'],data['cost'])
...
def load(file_path):
    ...
    model=MultilayerPerceptron(input_shape=tuple(data['input_shape']),
        output_shape=tuple(data['output_shape']),hidden_layers=tuple(data['hidden_layers']),
        activation=data['activation'],cost=data['cost'],
        batch_size=data['batch_size'],iter_max=data['iter_max'],
        learning_rate=data['learning_rate'],L2_alpha=data['L2_alpha'],
